...
class BookCommentServiceImplTest {
    ...
    void saveTest() {
        ...
        verify(bookCommentRepository).saveOrUpdate(argThat(actualSavedBookComment -> {
    ...
    @Test
    void findById() {
        when(bookCommentRepository.findById(1)).thenReturn(of(BookComment.builder().build()));
        when(bookCommentRepository.findById(2)).thenReturn(empty());
